...
try {
    ...
    if (includeSummary) {
        ...
        for (let i = 0; i < cards.length; i++) {
            const card = cards[i];
            const cy   = yPosition + i * (cardH + cardGap);

            // Fond de la carte
            doc.setFillColor(...TL);
            doc.setDrawColor(...TB);
            doc.setLineWidth(0.3);
            doc.roundedRect(margin, cy, cardW, cardH, 2, 2, "FD");

            // Point coloré à gauche (cercle) au lieu d'une barre superposée
            doc.setFillColor(...card.color);
            doc.circle(margin + 6, cy + cardH / 2, 2.5, "F");

            // Label
            doc.setFontSize(9);
            doc.setFont("helvetica", "normal");
            doc.setTextColor(100, 100, 100);
            doc.text(card.label, margin + 12, cy + cardH / 2 - 0.5);

            // Valeur en gras à droite
            doc.setFontSize(11);
            doc.setFont("helvetica", "bold");
            doc.setTextColor(...card.color);
            doc.text(card.value, margin + cardW - 5, cy + cardH / 2 + 3.5, { align: "right" });
        }

        yPosition += cards.length * (cardH + cardGap) + 10;
    }
    ...
    for (const monthKey of months) {
        ...
        // Titre du mois avec fond coloré léger — texte centré verticalement
        const monthBlockH = 10;
        doc.setFillColor(...TL);
        doc.setDrawColor(...TB);
        doc.setLineWidth(0.3);
        ...
        doc.setTextColor(...T);
        doc.setFont("helvetica", "bold");
        doc.text(`${monthName} ${year}`, margin + 4, yPosition + monthBlockH / 2 + 3);
        ...
        // ==================== RÉCAPITULATIF MENSUEL 1×4 ====================
        const monthSummary = calculatePeriodSummary(monthEntries);
        const mCardW   = pageWidth - margin * 2;
        const mCardH   = 14;
        const mCardGap = 3;
        const recapBlockH = 4 * (mCardH + mCardGap) + 10;

        if (yPosition + recapBlockH > pageHeight - margin) {
            doc.addPage();
            yPosition = margin;
        }
        ...
        const monthCards = [
            ...
            { label: "Jours travaillés",       value: `${monthSummary.daysWorked} jours`,           color: [21, 101, 192] }
        ];

        for (let i = 0; i < monthCards.length; i++) {
            const card = monthCards[i];
            const cy   = yPosition + i * (mCardH + mCardGap);

            // Fond de la carte
            doc.setFillColor(245, 245, 245);
            doc.setDrawColor(215, 215, 215);
            doc.setLineWidth(0.3);
            doc.roundedRect(margin, cy, mCardW, mCardH, 2, 2, "FD");

            // Point coloré à gauche
            doc.setFillColor(...card.color);
            doc.circle(margin + 6, cy + mCardH / 2, 2.5, "F");

            // Label
            doc.setFontSize(9);
            doc.setFont("helvetica", "normal");
            doc.setTextColor(100, 100, 100);
            doc.text(card.label, margin + 12, cy + mCardH / 2 - 0.5);

            // Valeur en gras à droite
            doc.setFontSize(11);
            doc.setFont("helvetica", "bold");
            doc.setTextColor(...card.color);
            doc.text(card.value, margin + mCardW - 5, cy + mCardH / 2 + 3.5, { align: "right" });
        }

        yPosition += monthCards.length * (mCardH + mCardGap) + 14;
    }
